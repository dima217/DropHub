...
export class UploadService {
  ...
  async uploadFileToRoom(params: UploadData) {
    ...
    await this.permissionClient.verifyUserAccess(userId, resourceId, resourceType, [
      AccessRole.ADMIN,
      AccessRole.WRITE,
    ]);

    const room = await this.roomService.getRoomById(resourceId);
    const fileExpiresAt = room?.expiresAt ? new Date(Date.now() + 24 * 60 * 60 * 1000) : null;
  ...
  async uploadFileToStorage(params: UploadData) {
    ...
    // Файлы в storage не имеют срока годности
    const fileUploadMeta = await this.fileService.createFileMeta({
      originalName,
      key,
      size: fileSize,
      mimeType,
      uploaderIp,
      expiresAt: null, // Файлы в storage не истекают
    });
